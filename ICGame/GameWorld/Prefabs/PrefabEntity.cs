	...
	public class PrefabEntity
	{
		...
		private void GameEntityCreateSceneNodes ( GameEntity entity )
		{
			...
			mainNode.AddAttachable( 
				new SimpleMeshEntity( Platform.Instance.Content.Load<Model>( _sceneNodesPrefab[Main].Mesh ), _sceneNodesPrefab[Main].Material )
			);
				...
				if ( !Main.Equals( scnp.Name ) )
				{
					...
					scn.AddAttachable(
						new SimpleMeshEntity( 
							Platform.Instance.Content.Load<Model>( _sceneNodesPrefab[scnp.Name].Mesh ), _sceneNodesPrefab[scnp.Name].Material 
						)
					);
